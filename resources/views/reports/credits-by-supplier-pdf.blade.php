...
<head>
    ...
    <title>Reporte de Créditos por Proveedor</title>
    <style>
        @page {
        ...
        <table class="header-table">
            <tr>
                <td class="header-brand" style="width: 55%;">
                    <h1>Reporte de Créditos por Proveedor</h1>
                    <p class="tagline">Cuentas por pagar y compromisos con proveedores</p>
    ...
    <table class="kpi-table">
        <tr>
            <td class="kpi-cell" style="width: 25%;">
                <div class="kpi-title">Compras a Crédito</div>
                <div class="kpi-value">{{ number_format(count($items)) }}</div>
                <div class="kpi-subtitle">Total compras registradas</div>
            </td>
            <td class="kpi-cell kpi-primary" style="width: 25%;">
                ...
                <div class="kpi-subtitle">Total financiado</div>
            </td>
            <td class="kpi-cell kpi-success" style="width: 25%;">
                ...
                <div class="kpi-subtitle">
                    {{ $totalCredit > 0 ? number_format(($totalPaid / $totalCredit) * 100, 1) : 0 }}% abonado
                </div>
            </td>
            <td class="kpi-cell kpi-danger" style="width: 25%;">
                <div class="kpi-title">Saldo por Pagar</div>
                <div class="kpi-value text-danger">${{ number_format($totalRemaining, 2) }}</div>
                <div class="kpi-subtitle">Deuda pendiente</div>
            </td>
        </tr>
    </table>
    ...
    <table class="invoice-table">
        <thead>
            <tr>
                ...
                <th class="text-right" style="width: 16%;">Saldo por Pagar</th>
            </tr>
        </thead>
        <tbody>
            @forelse($items as $item)
                ...
                @endphp
                <tr>
                    <td class="font-bold">{{ $item->purchase_number }}</td>
            ...
            @empty
                <tr><td colspan="6" class="empty-state">No se encontraron créditos a proveedores registrados</td></tr>
            @endforelse
        </tbody>
        @if(count($items) > 0)
        <tfoot>
            <tr class="subtotal-row">
                <td colspan="4" style="text-align: right; text-transform: uppercase;">Total General ({{ count($items) }} compras):</td>
