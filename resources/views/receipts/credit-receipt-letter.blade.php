...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .invoice { max-width: 740px; margin: 0 auto; padding: 12px 16px; }
        .invoice-header {
            display: flex;
        ...
        .invoice-number-value { font-size: 13px; font-weight: bold; color: #0066cc; }
        .invoice-date { font-size: 10px; color: #555; margin-top: 2px; }
        .info-row {
            display: flex;
            gap: 15px;
            margin-bottom: 10px;
        ...
        .info-value { font-size: 9.5px; color: #333; line-height: 1.35; }
        ...
        .payment-info { font-size: 9.5px; color: #444; line-height: 1.35; }
        .payment-info strong { color: #000; }
        .totals-box { width: 250px; background: #f8f8f8; border: 1px solid #ddd; padding: 6px 10px; }
        .total-row { display: flex; justify-content: space-between; font-size: 9.5px; padding: 1.5px 0; color: #444; }
        ...
        .cufe { font-size: 7.5px; word-break: break-all; color: #555; margin: 2px 0; }
        ...
        .seller-info { font-size: 8.5px; color: #888; margin-top: 4px; }
        .print-actions { position: fixed; top: 10px; right: 10px; display: flex; gap: 8px; z-index: 100; }
        .btn { padding: 8px 16px; font-size: 12px; font-weight: bold; border: none; border-radius: 8px; cursor: pointer; }
    ...
    <div class="invoice">
        ...
        <div class="invoice-header">
            <div>
                ...
                <div>
                    <img src="{{ Storage::url($sale->branch->logo) }}" alt="{{ $sale->branch->name }}" style="max-width: 150px; max-height: 55px; object-fit: contain;">
                </div>
                @endif
            </div>
        ...
        <table class="items-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 90px;" class="text-right">PRECIO</th>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @if($importCodeVisible && $item->product?->import_code)
                            <br><small style="color: #888; font-size: 8.5px;">Cód. Importación: {{ $item->product->import_code }}</small>
                        @endif
                        @if($item->discount_amount > 0)
                            @php
                                $discLabel = $item->discount_type === 'percentage' ? $item->discount_type_value . '%' : '$' . number_format($item->discount_type_value, 0);
                            @endphp
                            <br><small style="color: #888; font-size: 8.5px;">Desc: {{ $discLabel }}
        ...
        <div class="totals-section">
            ...
            <div class="payment-info">
                ...
                @if($sale->creditPayments->count() > 0)
                    <br><br><strong>Relación de Pagos:</strong>
                    @foreach($sale->creditPayments as $payment)
                        <br>{{ $payment->created_at->format('d/m/Y') }} - {{ $payment->paymentMethod->name ?? 'N/A' }}: ${{ number_format($payment->amount, 2) }}
                    @endforeach
                    @php $totalPaid = $sale->creditPayments->sum('amount'); @endphp
                    <br><strong>Total Pagado:</strong> ${{ number_format($totalPaid, 2) }}
                    <br><strong>Saldo Pendiente:</strong> <span style="color: red;">${{ number_format($sale->credit_amount - $totalPaid, 2) }}</span>
                @endif
            </div>
            @else
            <div></div>
