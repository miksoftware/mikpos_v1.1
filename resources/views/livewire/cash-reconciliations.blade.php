<div>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                @endif
                                @if($item->status === 'open' && auth()->user()->hasPermission('cash_movements.view'))
                                <button wire:click="openManageMovementsModal({{ $item->id }})"
                                    class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
    ...
    <!-- Manage Movements Modal -->
    @if($isManageMovementsModalOpen)
    <div class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            <!-- Backdrop -->
            <div class="fixed inset-0 transition-opacity bg-slate-900/75" wire:click="closeManageMovementsModal"></div>

            <!-- Modal -->
            <div class="relative z-10 w-full max-w-4xl mx-auto bg-white rounded-2xl shadow-xl max-h-[90vh] flex flex-col">
                <div class="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 rounded-t-2xl">
                    <h3 class="text-lg font-semibold text-slate-800">Gestión de Movimientos</h3>
                    <button wire:click="closeManageMovementsModal" class="text-slate-400 hover:text-slate-600">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>
                <div class="p-6 overflow-y-auto flex-1">
                    @if(count($manageMovements) > 0)
                        <div class="border border-slate-200 rounded-xl overflow-hidden">
                            <table class="w-full text-left text-sm">
                                <thead class="bg-slate-50 border-b border-slate-200">
                                    <tr>
                                        <th class="px-4 py-3 font-medium text-slate-500">Fecha</th>
                                        <th class="px-4 py-3 font-medium text-slate-500">Usuario</th>
                                        <th class="px-4 py-3 font-medium text-slate-500">Tipo</th>
                                        <th class="px-4 py-3 font-medium text-slate-500">Concepto</th>
                                        <th class="px-4 py-3 font-medium text-slate-500 text-right">Monto</th>
                                        <th class="px-4 py-3 font-medium text-slate-500 text-center">Acciones</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100">
                                    @foreach($manageMovements as $movement)
                                                ...
                                                <td colspan="6" class="px-4 py-4">
                                                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                                                        <div class="md:col-span-3">
                                                            ...
                                                            <select wire:model="editing_movement_type" class="w-full px-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white">
                                                                <option value="income">Ingreso</option>
                                                                <option value="expense">Egreso</option>
                                                            </select>
                                                            @error('editing_movement_type') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                                                        </div>
                                                        <div class="md:col-span-4">
                                                            <label class="block text-xs font-medium text-slate-700 mb-1">Concepto</label>
                                                            <input wire:model="editing_movement_concept" type="text" class="w-full px-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white">
                                                            @error('editing_movement_concept') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                                                        </div>
                                                        <div class="md:col-span-3">
                                                            <label class="block text-xs font-medium text-slate-700 mb-1">Monto</label>
                                                            <div class="relative">
                                                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">$</span>
                                                                <input wire:model="editing_movement_amount" type="number" step="0.01" min="0" class="w-full pl-7 pr-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white">
                                                            </div>
                                                            @error('editing_movement_amount') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                                                        </div>
                                                        <div class="md:col-span-2 flex gap-2 justify-end">
                                                            <button wire:click="cancelEditMovement" class="px-3 py-1.5 text-xs font-medium text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">Cancelar</button>
                                                            <button wire:click="updateMovement" class="px-3 py-1.5 text-xs font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors">Guardar</button>
                                                        </div>
                                                        <div class="md:col-span-12 mt-2">
                                                            <label class="block text-xs font-medium text-slate-700 mb-1">Notas (Opcional)</label>
                                                            <input wire:model="editing_movement_notes" type="text" class="w-full px-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white">
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
                                        @else
                                            <tr class="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td class="px-4 py-3">
                                                    @if($movement->type === 'income')
                                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-emerald-100 text-emerald-800">Ingreso</span>
                                                    @else
                                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">Egreso</span>
                                                    @endif
                                                </td>
                                                <td class="px-4 py-3">
                                                    <div class="font-medium text-slate-800">{{ $movement->concept }}</div>
                                                    @if($movement->notes)
                                                        <div class="text-xs text-slate-500 mt-0.5">{{ $movement->notes }}</div>
                                                    @endif
                                                </td>
                                                <td class="px-4 py-3 text-right font-medium {{ $movement->type === 'income' ? 'text-emerald-600' : 'text-red-600' }}">
                                                    {{ $movement->type === 'income' ? '+' : '-' }}${{ number_format($movement->amount, 2) }}
                                                </td>
                                                <td class="px-4 py-3 text-center">
                                                    <div class="flex items-center justify-center gap-2">
                                                        @if(auth()->user()->hasPermission('cash_movements.edit'))
                                                        <button wire:click="editMovement({{ $movement->id }})" class="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Editar">
                                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                                        </button>
                                                        @endif
                                                        @if(auth()->user()->hasPermission('cash_movements.delete'))
                                                        <button type="button" x-on:click="confirmAction('Esta acción no se puede deshacer.', () => $wire.deleteMovement({{ $movement->id }}), { title: '¿Eliminar este movimiento?' })" class="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Eliminar">
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <h4 class="text-slate-500 font-medium">No hay movimientos registrados</h4>
                            <p class="text-slate-400 text-sm mt-1">Aún no se han registrado ingresos ni egresos en este arqueo.</p>
                        </div>
                    @endif
                </div>
            </div>
