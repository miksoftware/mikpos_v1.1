<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()->hasPermission('roles.create')): ?>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-semibold text-slate-900"><?php echo e($role->display_name); ?></p>
                                    <p class="text-sm text-slate-500"><?php echo e($role->name); ?></p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-sm text-slate-600 max-w-xs truncate"><?php echo e($role->description ?: '-'); ?></p>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-sm font-medium bg-purple-100 text-purple-700"><?php echo e($role->permissions_count); ?></span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-sm font-medium bg-slate-100 text-slate-700"><?php echo e($role->users_count); ?></span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($role->name !== 'super_admin' && auth()->user()->hasPermission('roles.edit')): ?>
                                <button wire:click="toggleStatus(<?php echo e($role->id); ?>)" class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 <?php echo e($role->is_active ? 'bg-[#ff7261]' : 'bg-slate-200'); ?>">
                                    <span class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow transition duration-200 <?php echo e($role->is_active ? 'translate-x-5' : 'translate-x-0'); ?>"></span>
                                </button>
                                <?php elseif(!auth()->user()->hasPermission('roles.edit')): ?>
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-sm font-medium <?php echo e($role->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'); ?>">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <button wire:click="edit(<?php echo e($role->id); ?>)" class="p-2 text-slate-400 hover:text-[#ff7261] hover:bg-orange-50 rounded-lg transition-colors" title="Editar">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </button>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                <?php if(auth()->user()->hasPermission('roles.delete') && !$role->is_system): ?>
                                <button wire:click="confirmDelete(<?php echo e($role->id); ?>)" class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Eliminar">
        ...
        <div class="px-6 py-4 border-t border-slate-200"><?php echo e($roles->links()); ?></div>
        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
    </div>
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative w-full max-w-3xl bg-white rounded-2xl shadow-xl">
                    <div class="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
                        <h3 class="text-lg font-bold text-slate-900"><?php echo e($roleId ? 'Editar Rol' : 'Nuevo Rol'); ?></h3>
                    ...
                    <div class="px-6 py-4 max-h-[70vh] overflow-y-auto space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input wire:model="name" type="text" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#ff7261]/50 focus:border-[#ff7261] lowercase <?php echo e($is_system ? 'bg-slate-100 cursor-not-allowed' : ''); ?>" placeholder="nombre_rol" <?php echo e($is_system ? 'disabled' : ''); ?>>
                                ...
                                <div class="border border-slate-200 rounded-xl overflow-hidden">
                                    <div class="bg-slate-50 px-4 py-3 flex items-center justify-between">
                                        <div class="flex items-center gap-2">
                                            <span class="font-medium text-slate-700"><?php echo e($module->display_name); ?></span>
                                            <span class="text-sm text-slate-500">(<?php echo e($module->permissions->count()); ?> permisos)</span>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <button type="button" wire:click="selectAllModule(<?php echo e($module->id); ?>)" class="text-sm text-[#ff7261] hover:underline">Todos</button>
                                            <span class="text-slate-300">|</span>
                                            <button type="button" wire:click="deselectAllModule(<?php echo e($module->id); ?>)" class="text-sm text-slate-500 hover:underline">Ninguno</button>
                                        </div>
                                    </div>
                                    <div class="p-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $module->permissions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $permission): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <label class="flex items-center gap-2 cursor-pointer group">
                                            <input type="checkbox" wire:click="togglePermission(<?php echo e($permission->id); ?>)" <?php echo e(in_array($permission->id, $selectedPermissions) ? 'checked' : ''); ?> class="w-4 h-4 rounded border-slate-300 text-[#ff7261] focus:ring-[#ff7261]">
                                            <span class="text-sm text-slate-600 group-hover:text-slate-900"><?php echo e($permission->display_name); ?></span>
                                        </label>
...
<?php /**PATH C:\laragon\www\mikpos_v1.1\resources\views/livewire/roles.blade.php ENDPATH**/ ?>