...
<head>
    ...
    <title><?php echo e($title ?? 'Dashboard - MikPOS'); ?></title>
            ...
            <div class="flex flex-col items-start px-4 py-3 border-b border-white/10">
                <a href="<?php echo e(route('dashboard')); ?>" class="flex items-center gap-3">
                    ...
                    <span x-show="sidebarOpen" x-transition:enter="transition-opacity duration-200" x-transition:leave="transition-opacity duration-200"
                        class="text-lg font-bold text-white truncate max-w-[160px]"><?php echo e(auth()->user()->branch?->name ?? 'MikPOS'); ?></span>
                </a>
            </div>
...
$key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-489653547-0', null);
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="flex items-center gap-3 p-1.5 rounded-xl hover:bg-slate-100 transition-colors">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()->avatar): ?>
                                <img class="h-8 w-8 rounded-lg object-cover" src="<?php echo e(auth()->user()->avatar); ?>" alt="<?php echo e(auth()->user()->name); ?>" />
                            <?php else: ?>
                            ...
                            <div class="hidden md:block text-left">
                                <p class="text-sm font-medium text-slate-700"><?php echo e(auth()->user()->name); ?></p>
                                <p class="text-sm text-slate-500"><?php echo e(auth()->user()->roles->first()?->display_name ?? 'Sin rol'); ?></p>
                            </div>
        ...
        <aside x-show="mobileMenuOpen" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="flex flex-col px-4 py-3 border-b border-white/10 flex-shrink-0">
                <div class="flex items-center justify-between">
                    <a href="<?php echo e(route('dashboard')); ?>" class="flex items-center gap-3" @click="mobileMenuOpen = false">
                        <div class="w-10 h-10 bg-gradient-to-br from-[#ff7261] to-[#a855f7] rounded-xl flex items-center justify-center">
                            <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path>
                            </svg>
                        </div>
                        <span class="text-lg font-bold text-white truncate max-w-[140px]"><?php echo e(auth()->user()->branch?->name ?? 'MikPOS'); ?></span>
                    </a>
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                <?php if (isset($component)) { $__componentOriginal0aab876b288b74948d15144f4269b828 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal0aab876b288b74948d15144f4269b828 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.sidebar-menu','data' => ['mobile' => true]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('sidebar-menu'); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php if (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag): ?>
...
<?php /**PATH C:\laragon\www\mikpos_v1.1\resources\views/layouts/app.blade.php ENDPATH**/ ?>